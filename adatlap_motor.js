...
// finomításnál — az adatlap_teszt.html-hez nem kell hozzányúlni.
  ...
  function osszegyujtOsmTageket() {
    ...
    // nem tartó oszlopokhoz is használható -- a power tag értékét a felsovezetektarto
    // checkbox dönti el (alapból bepipálva = catenary_mast). Ha a mező még nincs kitöltve
    // (a ~3000 meglévő rekord nagy részénél még nincs), a korábbi, mindig fix "catenary_mast"
    // alapértelmezés marad érvényben -- enélkül a power tag hiányozna, és az oszlop eltűnne a
    // térkép normál rétegéből, mert a klasszifikáció ezen a tagen múlik.
    let tagek = { power: allapot['felsovezetektarto'] === false ? 'pole' : 'catenary_mast' };
  ...
  async function urlParameterbolElotolt() {
    ...
    // van tagelve, ez tükröződjön az adatlapon is (kipipálatlanul), ne csússzon vissza
    // catenary_mast-ra.
    allapot['felsovezetektarto'] = tagek.power !== 'pole';
