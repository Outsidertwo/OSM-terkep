...
// finomításnál — az adatlap_teszt.html-hez nem kell hozzányúlni.
  ...
  const BUILD_VERZIO = '2026-09-11d -- felsovezetek_tartas mező: eldönthető catenary_mast/pole mentéskor';
  ...
  function epitsFeszitesiTervSzoveget() {
    ...
    if (tipus === 'FA') return '(fa oszlopnak nincs feszítési terve)';
  ...
  function ertelmezCimkeSzoveget(szoveg) {
    ...
    // Fa oszlopnál nincs egyéb funkció/jelölés, ezért ha ezt felismerjük, a típuskód-
    // felismerést (alább) ki is hagyjuk -- a fa oszlop sosem kombinálódik T/K/KR/stb.
    // típussal vagy a tőcsavaros (C) jelöléssel.
    const elso = tokenek[0];
    const faIllesztes = elso.match(/^(.+\d)F$/);
    if (faIllesztes) {
      eredmeny.szam = faIllesztes[1];
      eredmeny.oszlop_tipus = 'FA';
    } else {
      eredmeny.szam = elso;
    }
    ...
    // típuskódnak és számnak is értelmeznénk egyszerre), és csak akkor, ha nem fa oszlopot
    // ismertünk fel fentebb.
    let tipusTalalt = false;
    if (!faIllesztes && tokenek.length >= 2) {
  ...
  function frissitGyerekLathatosag() {
    ...
    // Fa oszlopnak (oszlop_tipus === 'FA') nincs szerkezeti eleme, ezért a mérete
    // mező feleslegesen zavaró lenne -- ez a séma opciónkénti gyerek_mezok-ot nem
    // ismer, ezért itt, mezőszinten célzottan rejtjük el.
    const szerkezetiMeretEl = document.querySelector('#mezokKontener .mezo[data-kod="szerkezeti_elem_meret"]');
    if (szerkezetiMeretEl) {
      szerkezetiMeretEl.style.display = allapot['oszlop_tipus'] === 'FA' ? 'none' : '';
    }
    const foldelesMeretEl = document.querySelector('#mezokKontener .mezo[data-kod="foldeles_meret"]');
    if (foldelesMeretEl) {
      foldelesMeretEl.style.display = allapot['foldeles'] === 'egyedi' ? '' : 'none';
    }
  }

  function osszegyujtOsmTageket() {
    // Ez az adatlap felsővezeték-oszlopokhoz ÉS a rendszerhez tartozó, de felsővezetéket
    // nem tartó oszlopokhoz is használható -- a power tag értékét a felsovezetek_tartas
    // mező dönti el. Ha a mező még nincs kitöltve (a ~3000 meglévő rekord nagy részénél
    // még nincs), a korábbi, mindig fix "catenary_mast" alapértelmezés marad érvényben --
    // enélkül a power tag hiányozna, és az oszlop eltűnne a térkép normál rétegéből, mert a
    // klasszifikáció ezen a tagen múlik.
    let tagek = { power: allapot['felsovezetek_tartas'] === 'pole' ? 'pole' : 'catenary_mast' };
  ...
  // A tényleges címke-építő logika a cimke_epito.js-ben van (window.epitsCimkeHtml),
  // amit a térkép (index.html) is ugyanígy hív -- így a két hely sosem csúszhat szét,
  // és a másolt szöveg is helyesen tartalmazza a valódi szóköz-karaktereket.
  function epitsCimkeElonezetHtml() {
    const html = window.epitsCimkeHtml(allapot);
    return html || '<em style="color:#999;">(még nincs adat)</em>';
  }

  // Csak azokat a mezőket másolja át az allapot-ból, amelyek ténylegesen "elérhetők" --
  // vagyis minden felső szintű mező mindig bekerül, de egy gyerek-mező csak akkor, ha a
  // szülője aktív (checkbox be van pipálva, ill. radio/select-nél mindig, mert azoknak
  // mindig van kiválasztott értékük). Enélkül egy valaha bepipált, majd kikapcsolt (vagy
  // sosem valóban bepipált) checkbox gyerekeinek alapértelmezett értékei "szellemadatként"
  // bennragadnának a mentett rekordban és a popupban is (pl. "sulyos" kikapcsolva, de
  // "suly_tipus"/"sulykosar_tipus" mégis szerepel) -- ugyanaz a logika, mint amit az
  // osszegyujtOsmTageket() már helyesen alkalmaz az OSM-tagekre.
  function aktivMezoErtekek(mezok, cel) {
    mezok.forEach(m => {
      if (m.tipus === 'tartoszerkezet_lista') return; // a tartoszerkezetek tömböt a hívó adja hozzá külön
      if (m.kod in allapot) cel[m.kod] = allapot[m.kod];
      if (m.gyerek_mezok && m.gyerek_mezok.length > 0 && mezoAktiv(m)) {
        aktivMezoErtekek(m.gyerek_mezok, cel);
      }
    });
  }
  ...
  function formazottRekord() {
    const masolat = {};
    aktivMezoErtekek(sema.mezok, masolat);
    ...
    const { oszloplehorgonyzas, kivitel, la_alaptipus, la_meret, la_betonmennyiseg, ...tobbi } = masolat;
    ...
    Object.entries(tobbi).forEach(([kod, ertek]) => {
      ...
      if (kod === 'kihuzas_merteke') {
        vegleges['lehorgonyzas'] = { kivitel, alaptipus: la_alaptipus, meret: la_meret, betonmennyiseg: la_betonmennyiseg };
        beilleszve = true;
      }
    });
    if (!beilleszve) {
  ...
  function frissitElonezet() {
    document.getElementById('cimkeElonezet').innerHTML = epitsCimkeElonezetHtml();
  ...
  function alkalmazMentettRekordet(rekord) {
    ...
    if (lehorgonyzas) {
      ...
      if (lehorgonyzas.kivitel !== undefined) allapot['kivitel'] = lehorgonyzas.kivitel;
  ...
  async function urlParameterbolElotolt() {
    ...
    if (bejovoOsmId) {
      const sorEl = document.getElementById('osmNodeIdSor');
      if (sorEl) {
        sorEl.innerHTML = `OSM node ID: <b>${escapeHtml(String(bejovoOsmId))}</b> ` +
          `(<a href="https://www.openstreetmap.org/node/${escapeHtml(String(bejovoOsmId))}" target="_blank">megnyitás</a>)`;
      }
    }
    bejovoVonal = adat.vonal || null;
    bejovoVonalKod = adat.vonalKod || null;
    // A meglévő OSM power= tagből előtöltjük a döntést -- ha a node már helyesen "pole"-ra
    // van tagelve, ez tükröződjön az adatlapon is, ne csússzon vissza catenary_mast-ra.
    allapot['felsovezetek_tartas'] = tagek.power === 'pole' ? 'pole' : 'catenary_mast';
